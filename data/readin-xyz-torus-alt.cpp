...
int main()
{
	ifstream inFile("ARRAY_posr_new_0.dat");
	//ofstream outFile("ARRAY_posr_new.dat");
	   
	FILE * outfile = fopen("ARRAY_posr_new.dat", "w" );
	
	int i = 0;
	float variable1[MAX_LENGTH], variable2[MAX_LENGTH], variable3[MAX_LENGTH];
	...
	for(int j=0; j<i-1; j++)
	{
		cout << variable1[j] << " " << variable2[j] << " " << variable3[j] << endl;
	} //endfor
	cout << endl;
	...
	for(int j=0; j<i-1; j++)
	{
		fprintf(outfile, "%15.6f %14.6f %14.6f %8i \n", variable1[j], variable2[j], variable3[j], j+1);	
	}
	fclose(outfile); 
