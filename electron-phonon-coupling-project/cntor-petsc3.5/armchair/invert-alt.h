...
PetscErrorCode InvertHamiltonian (MPI_Comm PETSC_COMM_PASSED, Parameters& p, Mat hamiltonian, Mat inverseHamiltonian, Mat identityMat, PC pc, KSP ksp, 
								  int *isl180, int *isr180, int *isl, int *isr, complex<double> **gl, complex<double> **gr, double atomCoordinates[][3], double disCoordinates[][3])
{
	//Declarations
	time_t invStart;
	time(&invStart);
...
PetscErrorCode CalculateTransport (MPI_Comm PETSC_COMM_PASSED, Parameters& p, double& densityOfStates, complex<double> **gl, complex<double> **gr, 
								   complex<double> **gaml, complex<double> **gamrr, double IVvalues[], FILE* outFile)
{
	//Declarations
	time_t programStart;
	time(&programStart);
...
//					else cout << seconds << " seconds" << endl;
					...
					MPI_Recv(&IVvalues[currentStep+i-crossedZero], 1, MPI_DOUBLE, i*p.groupSize, 
							 currentStep+i-crossedZero, PETSC_COMM_PASSED, &MPIerr);
