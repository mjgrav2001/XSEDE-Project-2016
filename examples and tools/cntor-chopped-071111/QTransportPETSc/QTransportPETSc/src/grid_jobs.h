...
//This is the fundemental component of my code. A block holds a 12 by 12 data array of complex <double> types0.
...
//A block also has a unique id and the, more importantly, an int (mi) that should represent which column of the overall hmod it is a part of.
...
//is created, that the initialization routine is also intialized in set_values.
class block {
    int mi, id;
...
// Since the T,t-dagger,U, and U-dagger blocks are set once in the beginning, and then are never needed to be up dated, this
