...
PetscErrorCode InvertHamiltonian (MPI_Comm PETSC_COMM_PASSED, Parameters& p, Mat hamiltonian, Mat inverseHamiltonian, Mat identityMat, PC pc, KSP ksp, int *isl180, int *isr180, int *isl, int *isr, complex<double> **gl, complex<double> **gr, double atomCoordinates[][3], double disCoordinates[][3])
{
	...
	for(int j=p.myGroupRank*m; j<(p.myGroupRank+1)*m; j++)
	{
			...
			//blockHamiltonian.DetachLeads(p, j, 1, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);				
	}


	if(p.ndiag % p.myGroupSize != 0)
	{
		if(p.myGroupRank == 0)
		{
			for(int j=m*p.myGroupSize; j<p.ndiag; j++)
			{
					blockHamiltonian.set_zeros(p);
					blockHamiltonian.set_values(p, j, 1, 'D', atomCoordinates, disCoordinates);
					blockHamiltonian.AttachLeads(p, j, 1, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);			
	...
	for(int j=p.myGroupRank*m; j<(p.myGroupRank+1)*m; j++)
	{
		if(j<p.ndiag-1)
		{
			...
			//blockHamiltonian.DetachLeads(p, j, 2, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);				
		} //endfor
	}
	...
	if(p.ndiag % p.myGroupSize != 0)
	{
		if(p.myGroupRank == 0)
		{
			for(int j=m*p.myGroupSize; j<p.ndiag-1; j++)
			{
				blockHamiltonian.set_zeros(p);
				blockHamiltonian.set_values(p, j, 2, 'W', atomCoordinates, disCoordinates);
				blockHamiltonian.AttachLeads(p, j, 2, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);	
	...
	for(int j=p.myGroupRank*m; j<(p.myGroupRank+1)*m; j++)
	{
		...
		//blockHamiltonian.DetachLeads(p, j-1, 0, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 1);			
	}
	...
	if(p.myGroupRank == 0)
	{
		...
		//blockHamiltonian.DetachLeads(p, 0, 0, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);	
		...
		//blockHamiltonian.DetachLeads(p, p.ndiag-1, 2, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);	
	}
	...
	{	
		...
		for(int i=0; i<p.leadSize; i++) {
			...
			for(int j=0; j<p.leadSize; j++) {
				...
				//1.
				//cout <<  "isl[" << i << "] = " << isl[i] << endl;
				//cout <<  "isl[" << j << "] = " << isl[j] << endl;
		...
		for(int j=0; j<p.leadSize; j++) {
			...
			for(int i=0; i<p.leadSize; i++) {
				...
				//2.
				//cout <<  "isr[" << i << "] = " << isr[i] << endl;
				//cout <<  "isr[" << j << "] = " << isr[j] << endl;
