...
{
public:
	Parameters(PetscInt, PetscInt, PetscInt, double, double, double, double, double, double, int, double, double, int, int, int, double, PetscInt);
...
Parameters::Parameters(PetscInt inRows, PetscInt inColumns, PetscInt inIdim, double inEnergyMin, double inEnergyMax, double inEnergyStep, double inThop,
					   double inVsdMin, double inVsdMax, int inVsdSteps, double inVhop, double inBField, int inLeadSize, int inMtop, int inNtop, double inA, PetscInt inGroupSize)
{
	rows = inRows;
	columns = inColumns;
	idim = inIdim;
...
class block
{
 ...
 public:
	...
	void DetachLeads(int, const PetscInt, const PetscInt, const PetscInt, double, double, double, double,
					 block stm, int, int, int *isl180, int *isr180, int *isl, int *isr, 
					 complex<double> **gl, complex<double> **gr, 
					 double atomCoordinates[][3], double disCoordinates[][3],int);	
   ...
   int ic[itor+1];
	...
	switch(type)
	{
		...
		case 'U' :
			...
			//data[0][1] = bphs(1, 3590, p.BField, atomCoordinates) * vnew;
			//data[3][2] = bphs(4, 3591, p.BField, atomCoordinates) * vnew;
			//data[4][5] = bphs(5, 3594, p.BField, atomCoordinates) * vnew;
			//data[7][6] = bphs(8, 3595, p.BField, atomCoordinates) * vnew;
			//data[8][9] = bphs(9, 3598, p.BField, atomCoordinates) * vnew;
			//data[11][10] = bphs(p.idim, 3599, p.BField, atomCoordinates) * vnew;
			break;
		...
		case 'Q' :
			...
			//data[1][0] = bphs(3590, 1, p.BField, atomCoordinates) * vnew;
			//data[2][3] = bphs(3591, 4, p.BField, atomCoordinates) * vnew;
			//data[5][4] = bphs(3594, 5, p.BField, atomCoordinates) * vnew;
			//data[6][7] = bphs(3595, 8, p.BField, atomCoordinates) * vnew;
			//data[9][8] = bphs(3598, 9, p.BField, atomCoordinates) * vnew;
			//data[10][11] = bphs(3599, p.idim, p.BField, atomCoordinates) * vnew;
			break;
	}
} //end block.set_values


void block::AttachLeads(Parameters p, int aibloc, int ajbloc, int *isl180, int *isr180, int *isl, int *isr, complex<double> **gl, complex<double> **gr, 
						double atomCoordinates[][3], double disCoordinates[][3], int check)
{
...
void block::DetachLeads(int leadSize, const PetscInt rows, const PetscInt idim, const PetscInt ndiag, double vhop, double energy, double BField, double thop,
						block stm, int aibloc, int ajbloc, int *isl180, int *isr180, int *isl, int *isr, complex<double> **gl, complex<double> **gr, 
						double atomCoordinates[][3], double disCoordinates[][3], int check)
{
	int temp1, temp2, temp3, temp4,
		a1bloc, a2bloc, a3bloc, a4bloc,
	...
	for(int i=0; i<ig; i++)
	{
		for(int k=0; k<ig; k++)
		{
			...
			if(temp1 >= temp2)
			{
				a1bloc = (int) floor(temp1 / idim);
				a2bloc = get_atom_connect_block(idim, ndiag, temp2, temp1);
			} //endif
			else
			{
				a1bloc = (int) floor(temp2 / idim);
				a2bloc = get_atom_connect_block(idim, ndiag, temp2, temp1);	
			} //endelse
			...
			if ((aibloc == a1bloc)&&(ajbloc == a2bloc))
			{
				tempcd1 = stm.get_data(a1pl, a2pl);
				stm.set_data(a1pl, a2pl, tempcd1 + thop*thop*gl[i][k]);
				ham = stm.get_data(a1pl, a2pl);
				data[a1pl][a2pl] = ham;
			} //endif
			...
			if(temp3 >= temp4)
			{
				a3bloc = (int) floor(temp3 / idim);
				a4bloc = get_atom_connect_block(idim, ndiag, temp4, temp3);
			} //endif
			else
			{
				a3bloc = (int) floor(temp4 / idim);
				a4bloc = get_atom_connect_block(idim, ndiag, temp4, temp3);	
			} //endelse
			...
			if ((aibloc == a3bloc)&&(ajbloc == a4bloc))
			{
				tempcd2 = stm.get_data(a3pl, a4pl);
				stm.set_data(a3pl, a4pl, tempcd2 + thop*thop*gr[i][k]);
				ham = stm.get_data(a3pl, a4pl);
				data[a3pl][a4pl] = ham;
			} //endif
