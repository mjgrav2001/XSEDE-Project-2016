...
int main(int argc,char **args)
{
	...
	//Setting all parameters in a struct for easier passing to functions 
	//and a central location to make changes
	...
	//ReadInCoordinates(p.rows, disCoordinates);
	...
	//IS *indexSetRows, *indexSetColumns;
	//indexSetRows = new IS[numSubMatrices];
	//indexSetColumns = new IS[numSubMatrices];
	...
	//ierr = PetscFree(identityMat);CHKERRQ(ierr);
	//ierr = PetscFree(hamiltonian);CHKERRQ(ierr);
	//ierr = PetscFree(inverseHamiltonian);CHKERRQ(ierr);
