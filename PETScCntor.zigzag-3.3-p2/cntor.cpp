/* Program usage:  mpiexec ./cntor [-help] [all PETSc options] */

static char help[] = "Calculates tightbinding transport observables (retarded) Green's function G(E), T(E), D(E), current I for zigzag carbon nanotubes, -rings, (graphene nanoribbons).\n\n";
...
int main(int argc, char **args)
{
	...
	//ierr = MatSetFromOptions(identityMat);
	...
	//ierr = MatSetFromOptions(hamiltonian);
	ierr = MatSetUp(hamiltonian);CHKERRQ(ierr);
	
	//Setup PETSc inverseMat	
	ierr = MatCreate(PETSC_COMM_SUBGROUP, &inverseHamiltonian);CHKERRQ(ierr);
	ierr = MatSetType(inverseHamiltonian, MATDENSE);CHKERRQ(ierr);	
	...
	ierr = ISCreateGeneral(PETSC_COMM_SUBGROUP, p.leadSize, vsl, PETSC_COPY_VALUES, indexSetRows);CHKERRQ(ierr);
	ierr = ISCreateGeneral(PETSC_COMM_SUBGROUP, p.leadSize, vsr, PETSC_COPY_VALUES, indexSetColumns);CHKERRQ(ierr);
	
	if(numSubMatrices > 1) {
		ierr = ISCreateGeneral(PETSC_COMM_SUBGROUP, p.leadSize, vsr, PETSC_COPY_VALUES, indexSetRows+1);CHKERRQ(ierr);
		ierr = ISCreateGeneral(PETSC_COMM_SUBGROUP, p.leadSize, vsl, PETSC_COPY_VALUES, indexSetColumns+1);CHKERRQ(ierr);
	}	
	...
	if(p.numEnergySteps % p.numGroups != 0 && p.myGroupNum >= p.numEnergySteps % p.numGroups) {
