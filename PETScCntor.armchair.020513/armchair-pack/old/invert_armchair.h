								   ...
								   complex<double> **gaml, complex<double> **gamrr, double IVvalues[], FILE* outFile)
{
	...
	ierr = PetscSynchronizedFlush(PETSC_COMM_PASSED);CHKERRQ(ierr);
	...
	if(p.procsInUse > 1)
	{
		if(p.myWorldRank != 0)
		{
				if((p.myWorldRank % p.groupSize) == 0)
				{
					//cout << "rank " << p.myWorldRank << " sending " << currentStep*p.energyStep+p.energyMin << endl;
		...
		{ 
			...
			for(int i=1; (i < p.numGroups && currentStep+i*p.groupSize < p.numEnergySteps); i++)
			{
				...
				{
						//cout << "rank " << p.myWorldRank << " receiving " << (currentStep+i-crossedZero)*p.energyStep+p.energyMin << endl;
