...
Parameters::Parameters(PetscInt inRows, PetscInt inColumns, PetscInt inIdim, double inEnergyMin, double inEnergyMax, double inEnergyStep, double inThop, double inVsdMin, double inVsdMax, int inVsdSteps, double inVhop, double inBField, int inLeadSize, int inMtop, int inNtop, double inA, PetscInt inGroupSize)
{
	rows = inRows;
	columns = inColumns;
	idim = inIdim;
...
complex<double> bphs(int a1, int a2, double BField, double disCoordinates[][3])
{
	...
	//double rij;
	double cearg, searg, earg;
	complex<double> result;
	
	a11=a1-1;
	a21=a2-1;
	a1x = disCoordinates[a11][0];
	a1y = disCoordinates[a11][1];
	//a1z = disCoordinates[a11][2];
	a2x = disCoordinates[a21][0];
	a2y = disCoordinates[a21][1];
	//a2z = disCoordinates[a21][2];
	
	rhoij = (a1x + a2x)*(a1x + a2x) + (a1y + a2y)*(a1y + a2y);
	//rij = rhoij+(a1z + a2z)*(a1z + a2z);
...
{
	int temp1, temp2, temp3, temp4, a1bloc, a2bloc, a3bloc, a4bloc, a1pl, a2pl, a3pl, a4pl;
