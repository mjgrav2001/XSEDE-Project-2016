...
PetscErrorCode InvertHamiltonian (MPI_Comm PETSC_COMM_PASSED, Parameters& p, Mat hamiltonian, Mat inverseHamiltonian, Mat identityMat, PC pc, KSP ksp, int *isl180, int *isr180, int *isl, int *isr, complex<double> **gl, complex<double> **gr, double atomCoordinates[][3], double disCoordinates[][3])
{
	//Declarations
	time_t invStart;
	time(&invStart);
...
#ifndef NOMKL
	//Zero array our way if using dense
	PetscScalar *hamArr;
	if(p.useDense) {
	  MatDenseGetArray(hamiltonian, &hamArr);
	  for(int i = 0; i < p.columns * p.rows; i++) hamArr[i] = 0;
	  MatDenseRestoreArray(hamiltonian,&hamArr);
	}
#endif

	//*******************************************************************************************************
	int m = (int) p.ndiag / p.myGroupSize;
	...
	for(int j=p.myGroupRank*m; j<(p.myGroupRank+1)*m; j++)
	{
			...
			//blockHamiltonian.DetachLeads(p.leadSize, p.rows, p.idim, p.ndiag, p.vhop, p.energy, p.BField, p.thop, blockHamiltonian, j, 1, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);				
	}
	...
	for(int j=p.myGroupRank*m; j<(p.myGroupRank+1)*m; j++)
	{
		if(j<p.ndiag-1)
		{
			...
			//blockHamiltonian.DetachLeads(p.leadSize, p.rows, p.idim, p.ndiag, p.vhop, p.energy, p.BField, p.thop, blockHamiltonian, j, 2, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);				
		} //endfor
	}
	...
	for(int j=p.myGroupRank*m; j<(p.myGroupRank+1)*m; j++)
	{
		...
		//blockHamiltonian.DetachLeads(p.leadSize, p.rows, p.idim, p.ndiag, p.vhop, p.energy, p.BField, p.thop, blockHamiltonian, j-1, 0, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 1);			
	}
	...
	if(p.myGroupRank == 0)
	{
		...
		//blockHamiltonian.DetachLeads(p.leadSize, p.rows, p.idim, p.ndiag, p.vhop, p.energy, p.BField, p.thop, blockHamiltonian, 0, 0, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);	

		blockHamiltonian.set_zeros(p);
		blockHamiltonian.set_values(p, p.ndiag-1, 2, 'Q', atomCoordinates, disCoordinates);
		blockHamiltonian.AttachLeads(p, p.ndiag-1, 2, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);		
		for(int k=0; k<p.idim; k++)
		{
			for(int l=0; l<p.idim; l++)
			{
		...
		//blockHamiltonian.DetachLeads(p.leadSize, p.rows, p.idim, p.ndiag, p.vhop, p.energy, p.BField, p.thop, blockHamiltonian, p.ndiag-1, 2, isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, 0);	
	}
	...
	ierr = MatAssemblyEnd(hamiltonian, MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
        ...
        //puts("Assembled Hamiltonian");

#ifndef NOMKL
	if(p.useDense) {
		MatDenseGetArray(hamiltonian, &hamArr);

		int n = p.rows; //Convenience

		int lda = n; //Leading dimension
		int lwork = -1; //Find from MKL
		PetscScalar optwork; //Optimum lwork value from MKL
		int* ipiv = (int*) malloc(n*sizeof(int)); //Permutation matrix
		PetscScalar *work;
		int info; //TODO check, <0 error, >0 means element is 0
		
		if(p.useZh) {
		  char uplo = 'U'; //Upper LU factorization
		  zhetrf(&uplo,&n,hamArr,&lda,ipiv,&optwork,&lwork,&info); //Dry run
		  printf("Workspace size for factorization: %.0f\n",optwork.real());
		  lwork = (int) optwork.real();
		  work = (PetscScalar*) malloc(lwork*sizeof(PetscScalar));
		  zhetrf(&uplo,&n,hamArr,&lda,ipiv,work,&lwork,&info);
	  
		  zhetri(&uplo,&n,hamArr,&lda,ipiv,work,&info);

		} else {
		  zgetrf(&n,&n,hamArr,&lda,ipiv,&info);

		  zgetri(&n,hamArr,&lda,ipiv,&optwork,&lwork,&info); //Dry run
		  printf("Workspace size for inversion: %.0f\n",optwork.real());
		  lwork = (int) optwork.real();
		  work = (PetscScalar*) malloc(lwork*sizeof(PetscScalar));
		  zgetri(&n,hamArr,&lda,ipiv,work,&lwork,&info);
		}
		free(work);
		free(ipiv);
...
PetscErrorCode RetrieveInformation (Parameters& p, int& numSubMatrices, IS *indexSetRows, IS *indexSetColumns, Mat inverseHamiltonian, int *isl, int *isr, double& densityOfStates, complex<double> **gl, complex<double> **gr, complex<double> **gaml, complex<double> **gamrr)
{
	//Declarations
	time_t programStart;
	time(&programStart);
...
PetscErrorCode CalculateTransport (MPI_Comm PETSC_COMM_PASSED, Parameters& p, double& densityOfStates, complex<double> **gl, complex<double> **gr, complex<double> **gaml, complex<double> **gamrr, double IVvalues[], FILE* outFile)
{
	//Declarations
	time_t programStart;
	time(&programStart);
...
//		else cout << seconds << " seconds" << endl;
	...
	ierr = PetscSynchronizedFlush(PETSC_COMM_PASSED, stdout);CHKERRQ(ierr);

	
	if((p.myWorldRank % p.groupSize) == 0)
	{
		//Output to file
		ierr = PetscSynchronizedFPrintf(PETSC_COMM_PASSED, outFile, "%.5f \t%.5e\t (%.5e, %.5e)\t%.5e\n", p.energy, densityOfStates, scaledTransmissionFunction.real(), scaledTransmissionFunction.imag(), fermif.real());CHKERRQ(ierr);
...
//		else cout << seconds << " seconds" << endl;
	...
	ierr = PetscSynchronizedFlush(PETSC_COMM_PASSED, outFile);CHKERRQ(ierr);
	...
	if(p.procsInUse > 1)
	{
		if(p.myWorldRank != 0)
		{
			if((p.myWorldRank % p.groupSize) == 0)
			{
				MPI_Send(&IVvalue, 1, MPI_DOUBLE, 0, currentStep, PETSC_COMM_PASSED);
			}
		}
		else
		{
...
//					else cout << seconds << " seconds" << endl;
					...
					MPI_Recv(&IVvalues[currentStep+i-crossedZero], 1, MPI_DOUBLE, i*p.groupSize, currentStep+i-crossedZero, PETSC_COMM_PASSED, &MPIerr);
