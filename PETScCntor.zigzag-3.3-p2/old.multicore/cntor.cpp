...
int main(int argc,char **args)
{
	...
	cout <<  "Processors used for each PETSc matrix inversion: " << p.size0 << endl;
	...
	for(p.energy = p.energyMin + rankn * p.energyStep; p.energy < (p.energyMax+.9*p.energyStep); p.energy += p.energyStep * size1)
	{		
        //Skipping energy = 0
		if(abs(p.energy) >= .9*p.energyStep)
		{
			wiregf(p, isl180, isr180, isl, isr, gl, gr, disCoordinates);
			ierr = InvertHamiltonian(p, hamiltonian, inverseHamiltonian, identityMat, pc, ksp, 
									 isl180, isr180, isl, isr, gl, gr, atomCoordinates, disCoordinates, blockHamiltonian);CHKERRQ(ierr);
