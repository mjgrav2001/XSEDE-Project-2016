...
Parameters::Parameters(PetscInt inRows, PetscInt inColumns, PetscInt inIdim, double inEnergyMin, double inEnergyMax, double inEnergyStep, double inThop,
					   double inVsdMin, double inVsdMax, int inVsdSteps, double inVhop, double inBField, int inLeadSize, int inMtop, int inNtop, double inA, PetscInt insize0)
{
	rows = inRows;
	columns = inColumns;
	idim = inIdim;
...
public:
	...
	void set_zeros (const PetscInt, const PetscInt);
	...
	void AttachLeads(int, const PetscInt, const PetscInt, const PetscInt, double, double, double, double,
...
void block::set_zeros(const PetscInt idim, const PetscInt ndiag)
{
...
void block::AttachLeads(int leadSize, const PetscInt rows, const PetscInt idim, const PetscInt ndiag, double vhop, double energy, double BField, double thop,
						block stm, int aibloc, int ajbloc, int isl180[], int isr180[], int isl[], int isr[], complex<double> gl[4][4], complex<double> gr[4][4], 
						double atomCoordinates[][3], double disCoordinates[][3], int check)
{
	...
	for(int i=0; i<ig; i++)
	{
		for(int k=0; k<ig; k++)
		{
			...
			if(temp1 >= temp2)
			{
				a1bloc = (int) floor(temp1 / ndiag);
				a2bloc = get_atom_connect_block(idim, ndiag, temp2, temp1);
			} //endif
			else
			{
				a1bloc = (int) floor(temp2 / ndiag);
				a2bloc = get_atom_connect_block(idim, ndiag, temp2, temp1);	
			} //endelse
			...
			if(temp3 >= temp4)
			{
				a3bloc = (int) floor(temp3 / ndiag);
				a4bloc = get_atom_connect_block(idim, ndiag, temp4, temp3);
			} //endif
			else
			{
				a3bloc = (int) floor(temp4 / ndiag);
				a4bloc = get_atom_connect_block(idim, ndiag, temp4, temp3);	
			} //endelse
