...
//This is a simple function. Takes the value C and places at a,b in data. Note that a and b much be less than ndiag
	...
	switch(type)
	{
		case 'D' :
			...
			for(int i=1; i<p.ndiag; i++)
			{
				...
				ham = bphs(p.ndiag*mi+i-1+1, p.ndiag*mi+i+1, p.BField, disCoordinates) * vnew;		      
				data[i-1][i] = ham;
				//Below diagonal
				vnew = -p.vhop + vphon*delpos(p.ndiag*mi+i+1, p.ndiag*mi+i-1+1, atomCoordinates, disCoordinates);					
				ham = bphs(p.ndiag*mi+i+1, p.ndiag*mi+i-1+1, p.BField, disCoordinates) * vnew;
				data[i][i-1] = ham;
			} //endfor
		...
		case 'W' :
			...
			for(int j=1; j<p.ndiag; j+=2)
			{
				if(check == true)
				{
					...
					ham = bphs(mi*p.ndiag+j+p.ndiag, mi*p.ndiag+j+1, p.BField, disCoordinates) * vnew; //1,5,9
					data[j-1][j] = ham; //1,5,9
					check = !check;
				} //endif
				else if(check == false)
				{
					vnew = -p.vhop + vphon*delpos(mi*p.ndiag+j+p.ndiag+1, mi*p.ndiag+j, atomCoordinates, disCoordinates);		
				        ham = bphs(mi*p.ndiag+j+p.ndiag+1, mi*p.ndiag+j, p.BField, disCoordinates) * vnew; //3,7,11
					data[j][j-1] = ham; //3,7,11
					check = !check;
				} //endif
			} //endfor
			break;
		...
		case 'T' :
			...
			for(int j=1; j<p.ndiag; j+=2)
			{
				...
				{
					vnew = -p.vhop + vphon*delpos(mi*p.ndiag+j+p.ndiag, mi*p.ndiag+j+1, atomCoordinates, disCoordinates);	
					ham = conj(bphs(mi*p.ndiag+j+p.ndiag, mi*p.ndiag+j+1, p.BField, disCoordinates)) * vnew; //3,7,11
					data[j][j-1] = ham; //3,7,11
					check = !check;
				} //endif
				else if(check == false)
				  {
					vnew = -p.vhop + vphon*delpos(mi*p.ndiag+j+p.ndiag+1, mi*p.ndiag+j, atomCoordinates, disCoordinates);	
					ham = conj(bphs(mi*p.ndiag+j+p.ndiag+1, mi*p.ndiag+j, p.BField, disCoordinates)) * vnew; //1,5,9
					data[j-1][j] = ham; //1,5,9
					check = !check;
				} //endif
			} //endfor
		...
		case 'U' :
			...
			for(int j=1; j<=itor; j++)
			{
				if(check == true)
				{
					...
					ham = bphs(2*j-1, p.rows-p.ndiag+2*(j-1)+2, p.BField, disCoordinates) * vnew;
					data[ic[j]-1][ic[j]] = ham;
					check = !check;
				} //endif
				else if(check == false)
				{
					...
					ham = bphs(2*j, p.rows-p.ndiag+2*(j-1)+1, p.BField, disCoordinates) * vnew;
					data[ic[j]+1][ic[j]] = ham;
					check = !check;
				} //endif
			} //endfor
		...
		case 'Q' :
			...
			for(int j=1; j<=itor; j++)
			{
				if(check == true)
				{
					...
					ham = bphs(p.rows-p.ndiag+2*(j-1)+2, 2*j-1, p.BField, disCoordinates) * vnew;
					data[ic[j]][ic[j]-1] = ham;
					check = !check;
				} //endif
				else if(check == false)
				{
					...
					ham = bphs(p.rows-p.ndiag+2*(j-1)+1, 2*j, p.BField, disCoordinates) * vnew;
					data[ic[j]][ic[j]+1] = ham;
					check = !check;
				} //endif
			} //endfor
