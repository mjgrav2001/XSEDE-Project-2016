...
c example:
c 9440 atoms, 40 atoms per small rings (width), 240 atoms per large ring, 
c zigzag torus:
cc	bigr   = 120*1.42*3/2/2/pi = 40.6800
cc	smallr = 20*1.42*sqrt(3)/2/pi = 7.8196
	bigr   = 40.6800
	smallr = 7.8196

c 9520 atoms, 50 rings, 40 atoms per ring, (10,10) armchair torus:	
ccc	bigr   = 25*1.42*sqrt(3)/2/pi = 19.58
ccc	smallr = 20*1.42*3/2/2/pi = 6.765

c 19520 atoms, 300 rings, 40 atoms per ring, (10,10) armchair torus:	
c	bigr = 117.4d0
c	smallr = 6.765d0

c 10000 atoms, 250 rings, 40 atoms per ring, (10,10) armchair torus:	
c	bigr = 97.875d0
c	smallr = 6.765d0

c 9520 atoms, 238 rings, 40 atoms per ring, (10,10) armchair torus:
c outer radius R = bigr+smallr = 99.942	
c	bigr = 93.177d0
c	smallr = 6.765d0
...
c 8000 atoms, 200 rings, 40 atoms per ring, (10,10) armchair torus:
c	bigr = 78.3d0
c	smallr = 6.765d0
...
c 8000 atoms, 400 rings, 20 atoms per ring, (5,5) armchair torus:
c	bigr = 156.6d0
c	smallr = 3.383d0

c 4000 atoms, 200 rings, 20 atoms per ring, (5,5) armchair torus:
c	bigr = 78.3d0
c	smallr = 3.383d0
...
c 7200 atoms, 600 rings, 12 atoms per ring, (3,3) armchair torus:		
c	bigr = 234.8d0
c	smallr = 2.0d0
...
c		print 174,j,isr(j)	
c 174		format(3x,i6,3x,i6)
 	
 400	   continue		    

	   do 500 i=1,ig
	   do 500 j=1,ig
c		hmod(isl(i),isl(j))=html(i,j)-thop*thop*gl(i,j)
c		hmod(isr(i),isr(j))=htmr(i,j)-thop*thop*gr(i,j)
...
c 172		format(3x,i6,3x,i6)
		...
		do 550 i=1,ig 	   
		do 550 j=1,ig
			print 560,i,j,gaml(i,j),gamrr(i,j)
 560			format(3x,i6,3x,i6,3x,2(d10.4,3x),3x,2(d10.4,3x))	
 550 		continue			
 ...
 600		format(3x,d10.4,3x,d10.4,3x,2(d10.4,3x),3x,d10.4)

		write(unit=1339, fmt='(3x,d10.4,3x,d10.4,3x,2(d10.4,3x))') energy,dos,trans
...
**********************************************************************
	...
	parameter(idim=40)
	parameter(nrng=imax/idim)
	complex*16 unitm(nrng,nrng),testm(nrng,nrng)
	complex*16 g00(nrng,nrng),gjj(nrng,nrng),gkk(nrng,nrng)
	complex*16 gj0(nrng,nrng),gk0(nrng,nrng)
	complex*16 g0j(nrng,nrng),g0k(nrng,nrng)
	complex*16 gk0tor(nrng,nrng),g0ktor(nrng,nrng),g00tor(nrng,nrng)
	complex*16 g002(nrng,nrng),gkktmp(nrng,nrng)
	complex*16 gleft(imax,imax),gkktst(nrng,nrng)
	complex*16 d00(nrng,nrng),djj(nrng,nrng),dkk(nrng,nrng)
	complex*16 tkl(nrng,nrng),tlk(nrng,nrng)
	complex*16 ukl(nrng,nrng),ulk(nrng,nrng)
	complex*16 f1(nrng,nrng),f2(nrng,nrng)
	complex*16 f3(nrng,nrng),f4(nrng,nrng)
	complex*16 f(nrng,nrng),test(nrng,nrng)
	complex*16 prod(nrng,nrng),prod4(nrng,nrng)
	complex*16 hmod(imax,imax)
	data zip,one,two /0.0d0,1.0d0,2.0d0/
	common/hbloc/hmod
	common/gbloc/gleft
...
c       construct the unit matrix and the toroidal coupling blocks
 ...
 10		    continue
		    ...
		    do 12 i=1,nrng
		       do 12 j=1,nrng
			  ukl(i,j)=-hmod(i,j+nrng*ndm1)
 12			  ulk(i,j)=-hmod(i+nrng*ndm1,j)
...
c invert d00 and call it g00; this also comprises the upper nrng x nrng
c elements of the stored left disconnected greens functions gleft.
		
c				call cgetrf(nrng,nrng,g00,nrng,ipiv,info)
c       				call cgetri(nrng,g00,nrng,ipiv,work,nrng,info)
...
c this starts the iterative loop. write hmod into the d's and t's.  
					 ...
					 do 24 i=1,nrng
					    do 24 j=1,nrng
					       dkk(i,j)=hmod(i+nrngk,j+nrngk)
					       gkk(i,j)=dkk(i,j)
					       
					       tkl(i,j)=-hmod(i+nrng*km1,j+nrngk)
					       tlk(i,j)=-hmod(i+nrngk,j+nrng*km1)
 24					    continue
	
c invert dkk and copy it into gkk
				
c					   call cgetrf(nrng,nrng,gkk,nrng,ipiv,info)
c       				   call cgetri(nrng,gkk,nrng,ipiv,work,nrng,info)
...
c						   	call cgetrf(nrng,nrng,gkktmp,nrng,ipiv,info)
c       					   	call cgetri(nrng,gkktmp,nrng,ipiv,work,nrng,info)
...
c	call prmute(gj0,g0j)
 ...
 50							      continue
							      ...
							      do 55 i=1,nrng
								 do 55 j=1,nrng
								    gleft(i+nrngk,j+nrngk)=gkk(i,j)
								    gleft(i,j+nrngk)=g0k(i,j)
								    gleft(i+nrngk,j)=gk0(i,j)
 55								 continue
...
************************************************************************
	...
	complex*16 gleft(imax,imax), biggr(imax,imax),
     1             hmod(imax,imax)
	complex*16 grkk(nrng,nrng),grni(nrng,nrng),grnj(nrng,nrng)
	complex*16 grjj(nrng,nrng),grin(nrng,nrng),grjn(nrng,nrng)
	complex*16 sgjj(nrng,nrng),sgj0(nrng,nrng),sg0j(nrng,nrng)
	complex*16 gjj1(nrng,nrng),gjj2(nrng,nrng),
     1             gjj3(nrng,nrng),gjj4(nrng,nrng)
	complex*16 gnj1(nrng,nrng),gnj2(nrng,nrng),gjn1(nrng,nrng),
     1             gjn2(nrng,nrng), gnj3(nrng,nrng),gnj4(nrng,nrng),
     1             grnn(nrng,nrng)
	complex*16 god1(nrng,nrng),god2(nrng,nrng),god(nrng,nrng),
     1	grkj(nrng,nrng)
	complex*16 ulk(nrng,nrng),ukl(nrng,nrng)
	complex*16 tlk(nrng,nrng),tkl(nrng,nrng)
...
c outside the recursion.
	...
	do 12 i=1,nrng
	   do 12 j=1,nrng
	      ukl(i,j)=-hmod(i,j+nrng*ndm1)
	      ulk(i,j)=-hmod(i+nrng*ndm1,j)
 12	   continue

c fill up the greens function lower corner block, and the nrng by nrng matrices
c used in the iterations. note that the first iteraction  has grkk = grni = grin.

	   do 100 i=1,nrng
	      do 100 j=1,nrng
		 biggr(i,j)=gleft(i,j)
		 biggr(i+ndm1*nrng,j+ndm1*nrng)=gleft(i+ndm1*nrng,j+ndm1*nrng)
		 grkk(i,j)=gleft(i+ndm1*nrng,j+ndm1*nrng)
		 grni(i,j)=gleft(i+ndm1*nrng,j+ndm1*nrng)
		 grin(i,j)=gleft(i+ndm1*nrng,j+ndm1*nrng)   
 100		 grnn(i,j)=gleft(i+ndm1*nrng,j+ndm1*nrng)
c 	call prmute(grni,grin)
...
c start the backward iterations all the way up to the upper left corner element.
		 ...
		 do 200 k=ndm1,2,-1
		    ...
		    do 240 i=1,nrng
		       do 240 j=1,nrng
			  tkl(i,j)=-hmod(i+km1d,j+nrngk)
 240			  tlk(i,j)=-hmod(i+nrngk,j+km1d)

c	call prmute(tkl,tlk)
				...
				do 400 i=1,nrng
				   do 400 j=1,nrng			   
					biggr(km1d+i,km1d+j)=grjj(i,j)				      
				      	biggr(km1d+i,ndm1*nrng+j)=grjn(i,j)
				      	biggr(ndm1*nrng+i,km1d+j)=grnj(i,j)
 400 				continue
				       
 200				continue

c------------------------------------------------------------------------------
c New: Introduced my M. Jack, 10/15/2011:

				do 440 i=1,nrng
				   do 440 j=1,nrng
					biggr(i,j)=grjj(i,j)				      
				      	biggr(i,ndm1*nrng+j)=grjn(i,j)
				      	biggr(ndm1*nrng+i,j)=grnj(i,j)
 440 				continue

c------------------------------------------------------------------------------


c upward recursion from the idim-2 element all the way to the 11 element.
				   ...
				   if(ioffd.eq.1)then
					 ...
					 do 650 ic=k,1,-1
					    ...
					    do 700 j=1,nrng
						  ...
						  grkj(i,j)=biggr(i0+i,nrng*k+j)
						  sgj0(i,j)=gleft(i1+i,j)
						  grnj(i,j)=biggr(nrng*ndm1+i,nrng*k+j)
 700					    continue
					    ...
					    do 750 i=1,nrng
					    do 750 j=1,nrng				
						     biggr(i1+i,nrng*k+j)=god(i,j)
 						     biggr(nrng*k+j,i1+i)=dconjg(biggr(i1+i,nrng*k+j))
 750					    continue  
...
c
				do 555 i=1,imax
					print 560,i,biggr(i,i)
 560					format('test biggr(i,i)=const: ',3x,i6,3x,2(d10.4,3x))				       	
 555				continue
c
				  do 500 i=1,imax
 ...
 550					gamrr(i,j)=dconjg(biggr(isl(i),isr(j)))
c					gaml(i,j)=biggr(isl(i),isr(j))
c 550		     			gamrr(i,j)=dconjg(biggr(isr(j),isl(i)))					     						     
