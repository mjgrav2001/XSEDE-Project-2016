...
int main(int argc,char **args)
{
	...
	if(p.myWorldRank == 0) {
		...
		ierr = PetscPrintf(PETSC_COMM_WORLD, "Tightbinding transport calculation for zigzag carbon nanoring\n");CHKERRQ(ierr);
		ierr = PetscPrintf(PETSC_COMM_WORLD, "Total number of C-atoms: %d\n", p.rows);CHKERRQ(ierr);
		ierr = PetscPrintf(PETSC_COMM_WORLD, "Number of C-atoms around minor cirmcumference: %d\n", p.idim);CHKERRQ(ierr);
