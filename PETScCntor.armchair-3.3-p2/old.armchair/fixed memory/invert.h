		...
		for(int k=0; k<p.idim; k++)
		{
			for(int l=0; l<p.idim; l++)
			{
				if(blockHamiltonian[j][1].getData(k, l) != complex<double>(0,0))
					ierr = MatSetValue(hamiltonian, k+p.idim*j, l+p.idim*j, blockHamiltonian[j][1].getData(k, l), INSERT_VALUES);CHKERRQ(ierr);
			} //endfor
		} //endfor
	} //endfor
		...
		for(int k=0; k<p.idim; k++)
		{
			for(int l=0; l<p.idim; l++)
			{
				if(blockHamiltonian[j][2].getData(k, l) != complex<double>(0,0))
					ierr = MatSetValue(hamiltonian, k+p.idim*j, l+p.idim*(j-1), blockHamiltonian[j][2].getData(k, l), INSERT_VALUES);CHKERRQ(ierr);
				if(j == p.ndiag-2)
				{
					check = true;
					for(int i=1; i<=itor; i++)
					{
						if(check == true)
						{
							...
							if(blockHamiltonian[j][2].getData(ic[i],ic[i]+1) != complex<double>(0,0))
								ierr = MatSetValue(hamiltonian, p.rows-p.idim+2*(i-1), p.rows-2*p.idim+2*(i-1)+1, blockHamiltonian[j][2].getData(ic[i],ic[i]+1), INSERT_VALUES);CHKERRQ(ierr);
							check = !check;
						} //endif
						else
						{
							ic[i] = 2*i-1;
							if(blockHamiltonian[j][2].getData(ic[i],ic[i]-1) != complex<double>(0,0))
								ierr = MatSetValue(hamiltonian, p.rows-p.idim+2*(i-1)+1, p.rows-2*p.idim+2*(i-1), blockHamiltonian[j][2].getData(ic[i],ic[i]-1), INSERT_VALUES);CHKERRQ(ierr);
							check = !check;
						} //endelse
					} //endfor
				} //endif
		...
		for(int k=0; k<p.idim; k++)
		{
			for(int l=0; l<p.idim; l++)
			{
				if(blockHamiltonian[j][0].getData(k, l) != complex<double>(0,0))
					ierr = MatSetValue(hamiltonian, k+p.idim*(j-1), l+p.idim*j, blockHamiltonian[j][0].getData(k, l), INSERT_VALUES);CHKERRQ(ierr);
			} //endfor
		} //endfor
	} //endfor
	...
	for(int k=0; k<p.idim; k++)
	{
		for (int l=0; l<p.idim; l++)
		{
			if(blockHamiltonian[0][0].getData(k, l) != complex<double>(0,0))
				ierr = MatSetValue(hamiltonian, k, l+p.idim*(p.ndiag-1), blockHamiltonian[0][0].getData(k, l), INSERT_VALUES);CHKERRQ(ierr);
			if(blockHamiltonian[p.ndiag-1][2].getData(k, l) != complex<double>(0,0))
				ierr = MatSetValue(hamiltonian, k+p.idim*(p.ndiag-1), l, blockHamiltonian[p.ndiag-1][2].getData(k, l), INSERT_VALUES);CHKERRQ(ierr);
		} //endfor
	} //endfor
...
PetscErrorCode RetrieveInformation (Parameters p, Mat inverseHamiltonian, double IVvalues[], FILE* outFile, int isl[], int isr[], complex<double> gl[4][4], complex<double> gr[4][4])
{
	...
	complex<double> fermi1, fermi2, fermif, transmissionFunction;
	complex<double> scaledTransmissionFunction;
	double densityOfStates = 0, therm = .03;
	double emu1 = -p.vsdMax/2, emu2 = p.vsdMax/2;
	int currentStep = (p.energy < 0 ? (p.energy - p.energyMin) / p.energyStep + .5 : (p.energy - p.energyMin) / p.energyStep - .5);
	
	ierr = MatGetTrace(inverseHamiltonian, &trace);CHKERRQ(ierr);
	densityOfStates = -1 * trace.imag() / PI;
