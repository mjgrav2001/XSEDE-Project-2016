...
void InitGridOffDiag(Parameters p, block** stm, double** atomCoordinates, double** disCoordinates)
{
	for(int i=0; i<p.idim-1; i++)
	{
	...
	stm[p.idim-1][2].set_values(p.rows, p.idim, p.ndiag, p.vhop, p.idim-1, 2, p.energy, p.BField, 'Q', atomCoordinates, disCoordinates);
} //end InitGridOffDiag


//Sets up only the diagonal blocks
void InitGridDiag(Parameters p, block** stm, double** atomCoordinates, double** disCoordinates)
{
