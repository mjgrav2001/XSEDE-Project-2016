...
PetscErrorCode RetrieveInformation (Parameters p, Mat inverseHamiltonian, double IVvalues[], FILE* outFile, int isl[], int isr[], complex<double> gl[4][4], complex<double> gr[4][4])
{
	...
	PetscScalar trace;
	PetscInt nonZeros;
	PetscInt size, rank, sizen, rankn, sizesg, ranksg;
	PetscErrorCode ierr;
	...
	int ranks[p.size0];
	for (int l=0; l<p.size0; l++)
	{
		ranks[l] = ksg*p.size0 + l;
	}
	//******************************************************************************************************
	...
	if(p.size0 > 1) //parallel
	{
		...
		for(int i=0; i<p.leadSize; i++)
		{
			ierr = MatGetRow(localSubMatrices[k], i, &nonZeros, PETSC_NULL, &rowValues);CHKERRQ(ierr);
			for(int j=0; j<p.leadSize; j++)
				gaml[i][j] = rowValues[j];
			ierr = MatRestoreRow(localSubMatrices[k], i, &nonZeros, PETSC_NULL, &rowValues);CHKERRQ(ierr);
		} //endfor		
		
		for(int j=0; j<p.leadSize; j++)
		{
			ierr = MatGetRow(localSubMatrices[k], j, &nonZeros, PETSC_NULL, &rowValues);CHKERRQ(ierr);
			for(int i=0; i<p.leadSize; i++)
				gamrr[i][j] = conj(rowValues[i]);
			ierr = MatRestoreRow(localSubMatrices[k], j, &nonZeros, PETSC_NULL, &rowValues);CHKERRQ(ierr);
		} //endfor
		}
		
		//Determine permutation needed
		int isrPerm[p.leadSize], islPerm[p.leadSize];
		for(int i=0; i<p.leadSize; i++)
		{
			isrPerm[i] = 0;
			islPerm[i] = 0;
			...
			for(int j=0; j<p.leadSize; j++)
			{
				if(vsr[j] < vsr[i])	isrPerm[i]++;
				if(vsl[j] < vsl[i])	islPerm[i]++;
			}
		}
		...
		for(int i=0; i<p.leadSize; i++)
		{
			...
			if(islPerm[i] != i)
			{
				permuteRow(gaml, i, islPerm[i]);
				permuteRow(gamrr, i, islPerm[i]);
				tempVar = islPerm[i];
				islPerm[i] = i;
			...
			if(isrPerm[i] != i)
			{
				permuteColumn(gamrr, i, isrPerm[i]);
				permuteColumn(gaml, i, isrPerm[i]);
				tempVar = isrPerm[i];
				isrPerm[i] = i;
	...
	if(size > 1)
	{
		...
		{
			if(ranksg == 0)
				MPI_Send(&IVvalue, 1, MPI_DOUBLE, 0, currentStep, PETSC_COMM_SUBGROUP);
		}
		else
		{
			MPI_Status MPIerr;
			...
			for(int i=1; (i<sizen && currentStep+i<p.numEnergySteps); i++)
			{
				...
				{	
					if(ranksg == 0)
						MPI_Recv(&IVvalues[currentStep+i-crossedZero], 1, MPI_DOUBLE, i, currentStep+i-crossedZero, PETSC_COMM_SUBGROUP, &MPIerr);
				}
				else
					crossedZero = 1;
			} //endfor
