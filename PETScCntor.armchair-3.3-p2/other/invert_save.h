...
PetscErrorCode InvertHamiltonian (MPI_Comm PETSC_COMM_SUBGROUP, PetscInt size, PetscInt rank, PetscInt sizesg, PetscInt ranksg, 
...
PetscErrorCode RetrieveInformation (MPI_Comm PETSC_COMM_SUBGROUP, PetscInt size, PetscInt rank, PetscInt sizesg, PetscInt ranksg, 
									...
									complex<double> gl[4][4], complex<double> gr[4][4])
{
	...
	int n =  (int) size / p.size0;
	//int size0n = (int) p.size0 * n;
	int rank0 = (int) rank % p.size0;	
	...
	cout <<  " 2. " << endl;
	cout <<  " rank " << rank << " size " << size << endl;
	cout <<  " rank0 " << rank0 << " size0 " << p.size0 << endl << endl;
	...
	if(n > 1)
	{
		...
		else
		{
			...
			int crossedZero = 0;
			for(int i=1; (i < n && currentStep+i*p.size0 < p.numEnergySteps); i++)
			{
				//Skip Recv if energy = 0
                if(abs((currentStep+i*p.size0) * p.energyStep + p.energyMin) >= .9*p.energyStep)
				{	
					
					MPI_Recv(&IVvalues[currentStep+i*p.size0-crossedZero], 1, MPI_DOUBLE, i*p.size0, 
							 currentStep+i*p.size0-crossedZero, PETSC_COMM_WORLD, &MPIerr);
				}
				else
					crossedZero = 1*p.size0;
			} //endfor
		} //endelse
	} //endif
